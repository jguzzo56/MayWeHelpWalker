...
def Motors(onoff,effect_id):
    ...
    drv1.sequence[0] = adafruit_drv2605.Effect(effect_id)  # Set the effect on slot 0.
    ...
    drv1.sequence[1] = adafruit_drv2605.Pause(0.1)  # Pause for half a second
    ...
    if onoff == 1:
        drv1.play()  # play the effect
        drv2.play()  # play the effect
        drv3.play()  # play the effect  
        drv4.play()  # play the effect  
        # time.sleep(2)  # for 0.5 seconds // time seems to kill lidar - found github comments that agree
    else:
        drv1.stop()  # and then stop (if it's still running)
        drv2.stop()
        drv3.stop()
        drv4.stop()
...
def process_data(data, sensorstatus):
    ...
    for angle in range(360):
        ...
        elif distance < 914.4 and distance > 304.8:
        ...
        elif distance < 1828.8 and distance > 914.4:
             ...
             for scan in lidar.iter_scans():
                 ...
                 if scanCount == 5:                     
                     ...
                     if cond == 1:
                        ...
                        Motors(1,118)
                        print("condition 2<x<5")
                     elif 5 < cond <= 8:
                        Motors(1,121)
                        print("condition 5<x<8")
                     elif 8 < cond <= 11:
                        Motors(1,122)
                        print("condition 8<x<11")
                     else:
                        Motors(0,3)
                        print("off")
